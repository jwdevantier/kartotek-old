  ...
  (let [cursor (state/cursor [:modal] initial-state)]
    (fn []
      (let [{:keys [title body footer]} (current-component)
            hide? (nil? body)]
        [:div {:class ["modal" (when hide? "opacity-0") (when hide? "pointer-events-none") "fixed" "w-full" "h-full" "top-0" "left-0" "flex" "items-start" "justify-center"]}
         [:div {:class ["modal-overlay" "absolute" "w-full" "h-full" "bg-gray-900" "opacity-0"]}]
         [:div {:class "w-full h-full flex flex-col justify-start py-10"}
          [:div {:class ["modal-container" "flex" "relative" "w-10/12" "bg-x-grey" "mx-auto" "border-2" "border-x-grey-light" "z-50" "overflow-y-scroll" "scrollbar-thin" "scrollbar-thumb-x-blue" "scrollbar-track-x-grey-dark"]}
           ...
           [:div {:class ["modal-content" "flex" "py-4" "text-left" "px-6" ]}
