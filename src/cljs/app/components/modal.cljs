    ...
    (fn []
      (let [{:keys [title body footer]} (current-component)
        ...
        [:div {:class ["modal" (when hide? "opacity-0") (when hide? "pointer-events-none") "fixed" "w-full" "h-full" "top-0" "left-0" "flex" "items-start" "justify-center"]}
         ...
         [:div {:class "w-full h-full flex flex-col py-16"}
          [:div {:class ["modal-container" "relative" "w-10/12" "bg-x-grey" "mx-auto" "border-2" "border-x-grey-light" "z-50" "overflow-y-scroll" "scrollbar-thin" "scrollbar-thumb-x-blue" "scrollbar-track-x-grey-dark"]}
